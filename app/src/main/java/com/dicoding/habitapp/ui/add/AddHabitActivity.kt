...
class AddHabitActivity : AppCompatActivity(), TimePickerFragment.DialogTimeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        repository = HabitRepository.getInstance(applicationContext)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.action_save -> {
                val largestId = repository.getLargestHabitId() ?:0
                val id = largestId + 1
                ...
                if (title.isNotEmpty()) {
                    val habit = Habit(id = id, title = title, minutesFocus = minutesFocus, startTime = startTime, priorityLevel = priorityLevel)
                    viewModel.saveHabit(habit)
                    Log.d("Cek id Habbit", "$habitId")
                    finish()
                } else {
