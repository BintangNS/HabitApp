...
class RandomHabitActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.priorityLevelHigh.observe(this) { habit ->
            habit?.let { adapter.submitData(RandomHabitAdapter.PageType.HIGH, it) }
        }
        viewModel.priorityLevelMedium.observe(this) { habit ->
            habit?.let { adapter.submitData(RandomHabitAdapter.PageType.MEDIUM, it) }
        }
        viewModel.priorityLevelLow.observe(this) { habit ->
            habit?.let { adapter.submitData(RandomHabitAdapter.PageType.LOW, it) }
        }
